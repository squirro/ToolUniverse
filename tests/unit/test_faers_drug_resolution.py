...
Three defects in how `FAERSAnalyticsTool` turns a drug name into a query.

**1. A brand name resolves to nothing.** Only `patient.drug.openfda.generic_name` was
searched. Measured against openFDA 2026-08-03: `generic_name:"Lutathera"` -> HTTP 404,
`brand_name:"Lutathera"` -> 5,551 reports, `medicinalproduct.exact:"LUTATHERA"` -> 5,550,
`generic_name:"LUTETIUM LU 177 DOTATATE"` -> 5,551. So the tool answered "Insufficient
data: a=0, b=0" for a drug with 5,551 reports.

**2. The case definition was never stated.** `generic_name` lands on the NARROW definition;
the union of all reported spellings is 5,783 and moves myelodysplastic syndrome from 29
cases to 50 and the ROR from 7.2 to 12.0, while renal impairment is stable at 2.22 either
way -- so the sensitivity is signal-specific and cannot be assumed away.

**3. A transport failure was indistinguishable from a real zero.** `_get_faers_count` ended
in `except Exception: return 0`, which tripped the `a <= 0` guard and surfaced as
"Insufficient data" -- the same message a genuinely absent drug produces.

The fix resolves through a documented field chain, reports which field and how many reports
it matched, and lets a transport failure raise instead of becoming a number.
"""
...
def _shipped_config():
    ...
    for path in glob.glob(
        str(pathlib.Path(__file__).resolve().parents[2]
            / "src" / "tooluniverse" / "data" / "**" / "*.json"), recursive=True):
        try:
...
@pytest.mark.unit
@pytest.mark.parametrize("answers,name,resolved,calls", [
    # `generic_name` alone reported 5,551 reports as no data at all.
    (lambda field: 5551 if "brand_name" in field else None, "Lutathera", (BRAND, 5551), None),
    # The common path must not change: no extra requests, same field.
    (lambda field: 5551 if "generic_name" in field else None, "LUTETIUM LU 177 DOTATATE",
     (GENERIC, 5551), [GENERIC]),
    # The fallback chain must not invent a match.
    (lambda field: None, "notadrug", (None, None), None),
])
def test_a_drug_name_resolves_through_the_documented_field_chain(monkeypatch, answers, name,
                                                                 resolved, calls):
    seen = []
    monkeypatch.setattr(FAERSAnalyticsTool, "_field_total",
                        lambda self, field, term: seen.append(field) or answers(field),
                        raising=False)

    assert _tool()._resolve_drug_field(name) == resolved
    # generic_name must still be tried FIRST, so existing numbers do not move.
    assert seen[0] == GENERIC, seen
    assert calls is None or seen == calls, (
...
def test_the_envelope_states_which_field_and_how_many_reports(monkeypatch):
    ...
    monkeypatch.setattr(FAERSAnalyticsTool, "_get_faers_count",
                        ...
                        29 if (drug and event) else (5551 if drug else 14805), raising=False)
    ...
    cd = result["data"].get("case_definition")   # run() wraps the operation's dict under data
    assert cd, f"no case_definition in the envelope: {list(result['data'])}"
    assert cd["resolved_field"] == BRAND, cd
