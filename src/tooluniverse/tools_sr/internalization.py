...
ENDOCYTIC_PARTNERS = frozenset({
    "AP2A1", "AP2B1", "AP2M1", "AP2S1", "CLTC", "CLTA",
    "DNM1", "DNM2", "ARRB1", "ARRB2", "RAB5A", "RAB5B",
    "RAB5C", "RAB7A", "EPS15", "HIP1",
})
...
def fetch_uniprot_topology(gene_symbol, timeout=20):
    ...
    try:
        ...
        location_values = [(sl.get("location") or {}).get("value", "") for sl in sub_locs]
...
def fetch_endocytic_interactors(gene_symbol, timeout=20):
    ...
    import requests as _rq
    try:
        r = _rq.get("https://string-db.org/api/json/interaction_partners", params={
            "identifiers": gene_symbol, "species": "9606", "limit": "50",
        }, timeout=timeout)
        r.raise_for_status()
        seen = set()
        result = []
...
def assess(gene_symbol, uniprot_data, interactors, lit_count):
    ...
    def _ecd_length(ud):
        ...
        extra = [d for d in topo_doms if "extracellular" in (d.get("description") or "").lower()]
    ...
    def _extract_tail(ud):
        ...
        cyto = [d for d in topo_doms if "cytoplasmic" in (d.get("description") or "").lower()]
        if not cyto or not seq_str: return None
        best = max(cyto, key=lambda d: (_safe_int((d.get("location") or {}).get("end", {}).get("value", 0))
    ...
    def _scan_motifs(tail_seq):
        if not tail_seq or len(tail_seq) < 4: return []
        motifs = []
        bulky = set("FILMVW")
        for i in range(len(tail_seq) - 3):
            sub = tail_seq[i:i+4]
            if sub[0] == "Y" and sub[3] in bulky: motifs.append({"motif": "YXXΦ", "pattern": sub})
        if len(tail_seq) >= 6:
            for i in range(len(tail_seq) - 5):
                sub = tail_seq[i:i+6]
                if sub[0] in "DE" and sub[4] == "L" and sub[5] in "LI": motifs.append({"motif": "[DE]XXXL[LI]", "pattern": sub})
        for i in range(len(tail_seq) - 3):
            sub = tail_seq[i:i+4]
            if sub[0] == "N" and sub[1] == "P" and sub[3] == "Y": motifs.append({"motif": "NPXY", "pattern": sub})
        if len(tail_seq) >= 5:
            for i in range(len(tail_seq) - 4):
                sub = tail_seq[i:i+5]
                if sub[0] == "M" and sub[4] in "LI": motifs.append({"motif": "MXXXL", "pattern": sub})
        return motifs

    def _ceiling(tm, gpi):
        if gpi: return 0.40
        return {0: 1.0, 1: 1.0, 2: 0.55, 3: 0.55, 4: 0.45, 5: 0.70, 6: 0.70, 7: 0.85}.get(tm, 0.65)

    def _baseline(tm, gpi):
        if gpi: return 0.15
        return {0: 0.0, 1: 0.0, 2: 0.20, 3: 0.20, 4: 0.15, 5: 0.35, 6: 0.35, 7: 0.55}.get(tm, 0.25)

    def _topo_label(tm, gpi, intracellular=False):
        if gpi: return "GPI-anchored"
        if intracellular: return "cytoplasmic"
        if tm == 0: return "soluble"
        if tm == 1: return "single-pass"
        if tm == 7: return "7TM-GPCR"
        return f"{tm}TM"
    ...
    _intracellular_kws = ("cytoplasm", "nucleus", "cytosol", "mitochondri",
                          "endoplasmic reticulum", "golgi apparatus", "peroxisome")
    _membrane_kws = ("cell membrane", "plasma membrane", "membrane", "secreted",
                     "cell surface", "extracellular")
    is_intracellular = (
        tm_count == 0 and not is_gpi and bool(_sub_locs_str)
    ...
    has_lys = bool(tail_info and tail_info.get("tail_sequence") and "K" in tail_info["tail_sequence"])

    topo_score = 2 if (not is_gpi and tail_length >= 10) else (1 if (not is_gpi and tail_length >= 5) else 0)
    ...
    lit_score = (2 if lit_count >= 10 else (1 if lit_count >= 3 else 0)) * (0.5 if is_multi else 1)
    ...
    evidence = []
    if is_gpi: evidence.append("GPI-anchored (caveolar route only)")
    if is_multi: evidence.append(f"Multi-pass TM ({tm_count} segments)")
    if is_7tm: evidence.append("7-TM GPCR")
    if has_arrestin: evidence.append(f"Arrestin: {', '.join(i for i in interactors if i in {'ARRB1','ARRB2'})}")
    if tail_length >= 10: evidence.append(f"Tail: {tail_length} aa")
    if motifs: evidence.extend(f"{m['motif']}({m['pattern']})" for m in motifs)
    if interactors: evidence.append(f"STRING: {', '.join(interactors)}")
    if lit_count > 0: evidence.append(f"EPMC: {lit_count} papers")

    has_endo = bool(motifs) or bool(interactors)
    has_strong = (bool(motifs) and (bool(interactors) or len(motifs) >= 2)) or (lit_count >= 50 and has_endo)
    _IMPAIRED = {"ERBB2", "ERBB3", "MUC1", "CD33"}

    if is_intracellular:
        # Confirmed cytoplasmic/nuclear protein — cannot internalize as surface target
        norm = 0.0
        evidence.insert(0, "cytoplasmic/nuclear (no membrane anchor — UniProt confirmed)")
    elif is_gpi and lit_count >= 50: norm = min(0.55, max(0.40, base + raw_norm * (0.55 - base)))
    elif is_gpi and lit_count >= 10: norm = min(ceil, max(0.3, base + raw_norm * (ceil - base)))
    elif gene_symbol.startswith("CLDN") and tm_count == 4: norm = min(0.72, 0.30 + raw_norm * 0.42)
    elif gene_symbol in _IMPAIRED: norm = min(0.70, max(0.50, raw_norm * 0.75))
    elif lit_count >= 10 and not is_multi and not is_gpi and topo_score >= 1 and has_strong: norm = min(ceil, max(0.8, raw_norm))
    elif lit_count >= 10 and not is_multi and not is_gpi and topo_score >= 1 and has_endo: norm = min(ceil, max(0.5, raw_norm))
    elif lit_count >= 10 and is_multi: norm = min(ceil + 0.10, base + raw_norm * (ceil - base) * 1.1)
    elif is_multi: norm = min(ceil, base + raw_norm * (ceil - base))
    else: norm = min(ceil, raw_norm)
    ...
    return {
        "score": min(1.0, float(norm)), "raw_score": raw, "evidence": evidence,
        "tail_length": tail_length, "motifs": motifs, "interactors": interactors,
        "literature_count": lit_count, "is_gpi": bool(is_gpi), "multi_pass": is_multi,
        "tm_count": tm_count, "gpcr_boost": gpcr_boost, "ceiling": ceil, "baseline": base,
        "ecd_length": ecd, "topology_class": _topo_label(tm_count, is_gpi, is_intracellular),
...
def exec_internalization(arguments, input_table, output_table, db_path):
    ...
    log.info("exec_internalization: %d genes from column '%s', threshold=%.2f", len(genes), gene_col or "arguments", threshold)

    def _score_one(gene):
        time.sleep(0.1)
        uniprot = fetch_uniprot_topology(gene) or {
            "is_gpi_anchored": False, "transmembrane_features": [],
            "topological_domains": [], "sequence": None, "subcellular_locations": [],
        }
        ...
        return {
            ...
            "motifs": "; ".join(f"{m['motif']}({m['pattern']})" for m in result["motifs"]) if result["motifs"] else "",
            "endocytic_interactors": "; ".join(result["interactors"]) if result["interactors"] else "",
    ...
    log.info("exec_internalization: %d/%d genes scored above threshold %.2f",
             len(results), len(genes), threshold)
    return results
