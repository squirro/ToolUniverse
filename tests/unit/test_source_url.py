...
query parameters, so the link that exists often lands on a bare endpoint rather than the
query that produced the answer. The DSR-658 interceptor already holds the fully resolved
URL, so this is mostly about choosing which call to cite and stripping what must never be
published. Redaction is a hard requirement, not polish: 24 modules pass ``api_key`` and
several put ``email``/``password`` in the query string, so the redactor is deliberately
aggressive -- degrading a link is cheap, publishing a key is not.
"""

import pytest

from tooluniverse.tools_sr import http_record, source_url
from tooluniverse.tools_sr.source_url import scrub

GRAPHQL = "https://api.platform.opentargets.org/api/v4/graphql"
ENSEMBL = "https://rest.ensembl.org/lookup/symbol/human/SSTR2?content-type=json"
...
@pytest.mark.parametrize("param", ["api_key", "apiKey", "apikey", "API_KEY", "access_token",
                                   "token", "email", "password", "client_secret", "secret"])
def test_a_credential_parameter_never_survives_into_the_stamped_url(param):
    ...
    redacted = source_url.redact(f"https://x.org/q?search=aspirin&{param}=someone%40example.com")

    assert "someone" not in redacted, redacted
...
@pytest.mark.parametrize("url,kept", [
    ...
    ("https://x.org/q?api_key=abc&gene=TP53", ["api_key=REDACTED", "gene=TP53"]),
    # 'keyword' is not a credential; a blunt substring rule would break real queries.
    ("https://x.org/search?keyword=kinase&monkey=1", ["keyword=kinase", "monkey=1"]),
])
def test_the_parameter_is_kept_and_only_a_credential_value_is_masked(url, kept):
    redacted = source_url.redact(url)

    assert all(part in redacted for part in kept), redacted
...
# --- which call gets cited (DSR-631) ---


@pytest.mark.parametrize("records,expected", [
    # A POST's URL does not identify the query: all OpenTargets tools POST to one endpoint,
    # so stamping it would produce a footnote that looks checked and lands somewhere useless.
    ([_record(GRAPHQL, method="POST")], None),
    # A tool that resolves over REST then queries GraphQL can still cite the REST call.
    ([_record(ENSEMBL), _record(GRAPHQL, method="POST")], ENSEMBL),
    # The last successful call is cited, not the first: cite the query, not the lookup.
    ([_record("https://x.org/resolve?symbol=TP53"),
      _record("https://x.org/associations?id=ENSG00000141510")],
     "https://x.org/associations?id=ENSG00000141510"),
    # A failed call is never cited.
    ([_record("https://x.org/good?a=1"), _record("https://x.org/dead?b=2", reached=False)],
     "https://x.org/good?a=1"),
    ([_record("https://x.org/dead", reached=False)], None),
    ([], None),
])
def test_which_call_gets_cited(records, expected):
    assert source_url.pick(records) == expected


# --- stamping ---


@pytest.mark.parametrize("result,records", [
    # A GraphQL-only tool: the POST is not citable, and the result still comes back whole.
    ({"associations": [{"id": 1}]}, [_record(GRAPHQL, method="POST")]),
    # A tool making no HTTP call at all.
    ({"answer": "computed locally"}, []),
])
def test_a_result_with_nothing_to_cite_is_well_formed_with_no_source_url(result, records):
    stamped = source_url.stamp(result, records)
...
def test_a_result_that_already_cites_its_source_is_left_alone():
    ...
    stamped = source_url.stamp({"rows": [], "source_url": "https://x.org/curated/view"},
                               [_record("https://x.org/api?raw=1")])
...
# carried `api_key=<real key>` in the `error` field, through the agent's trace, into a
# committed report. `source_url` was redacted; nothing else was.
...
def test_the_wrapper_scrubs_before_it_stamps():
    """Through the installed wrapper: an error result with a key in its text comes back
    masked even when no source URL can be cited."""
    class Registry:
        all_tool_dict = {}

        def run_one_function(self, function_call_json):
            return {"status": "error",
                    "error": "GET https://x.test/?api_key=SECRET12345678901234 failed"}
