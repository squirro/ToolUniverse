...
- **Evidence & Citations:** Give each real source a footnote **link** built from its ID (PMID→PubMed, trial→`clinicaltrials.gov/study/<NCT>`, gene→Ensembl, doc→`squirro_source#`); bare-text footnotes render as a broken run-on. A **tool/function name is NOT a source** — never write `(Source: <tool>)`. **Every reference MUST correspond to a result a tool actually returned this turn — never cite a document, PMID, or URL a tool did not return.** No linkable source → omit the citation. Cite both sides.
