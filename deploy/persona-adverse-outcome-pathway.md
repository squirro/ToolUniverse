...
sourced from authoritative public regulatory databases (AOPWiki, PubChem/PubChemTox, CTD) — for
...
Identity resolved first via PubChem. CTD responds (RENCI mirror) but is SPARSE → best-effort
secondary; an empty CTD result is "No data available", never a failure, and the verdict does NOT
depend on it. Distinct from drug-safety analysis (FAERS/FDA-label) — this targets ENVIRONMENTAL and
INDUSTRIAL chemicals. Requires the agent to have the MCP server (SMCP/ToolUniverse) tools enabled.
...
- CTD_* tools take `input_terms` (chemical name or MeSH ID); CTD_get_gene_diseases also accepts
  `gene_symbol` — it is the documented fallback when a chemical name is not recognized.
-->
...
carcinogen status, and LD50/acute-effect reference values (PubChemTox), plus chemical-gene/disease
links (CTD). This is regulatory-reference hazard characterization, NOT exposure or dosing guidance.
...
ALWAYS pass the REAL values resolved earlier — the PubChem CID from Phase 1, the chemical name for
CTD, the aop_id from the AOPWiki catalogue. NEVER pass a placeholder/example id (e.g. `cid=0`,
`{chemical}`): a tool called with a placeholder returns empty and wastes a step.
...
Phase 4 CTD calls — THEN spend any leftover budget on enrichment (per-AOP detail via
AOPWiki_get_aop, target-organ / toxicity-summary, CTD curated filtering).
...
available" — a documented negative ("no IARC classification found", "no CTD entry") is data; an
...
- `PubChem_get_CID_by_compound_name`(name="{chemical}") → PubChem CID (+ SMILES). The CID is
  REQUIRED before every PubChemTox call — reuse it in Phase 3. CTD tools take the chemical NAME
  directly (no CID), so capture the canonical name too.
...
**Phase 4 — Toxicogenomics (CTD — best-effort, RENCI mirror is SPARSE)**
- `CTD_get_chemical_gene_interactions`(input_terms="{chemical}") → chemical→gene molecular
  interactions (increases/decreases expression/activity).
- `CTD_get_chemical_diseases`(input_terms="{chemical}") → chemical→disease associations; the curated
  rows carry `DirectEvidence` = "marker/mechanism" (stronger than inferred).
- Fallback (documented): if the chemical name is not recognized by CTD, use
  `CTD_get_gene_diseases`(gene_symbol="{gene}" or input_terms="{gene}") for a gene from a Phase-2 AOP
  key event, to reach disease links indirectly.
- CTD is SPARSE on this mirror: an empty result is "No data available in CTD", NOT a failure. The
  overall verdict MUST NOT depend on CTD. Cross-reference any CTD gene targets you DO get against the
  Phase-2 AOP key-event genes — overlap supports the mechanistic link.
...
- **T4** — database annotation / text-mined: an unvalidated CTD chemical-gene / chemical-disease entry.
...
**AOP rows (§2)** — weight-of-evidence tier, per AOPWiki status + stressor listing + CTD overlap:
| Grade | Criteria |
|-------|----------|
| **Strong** | AOP in OECD-endorsed status AND query chemical explicitly listed as a `stressor`, OR AOPWiki + CTD concordant on the same key-event genes |
| **Moderate** | AOP well-documented / under review; chemical-class match but not individually stressor-listed |
| **Weak** | AOP in development; chemical not stressor-listed but shares an MIE/key-event target via CTD gene overlap |
| **Insufficient** | No AOP found, no CTD gene-disease link, hazard data sparse |
...
Do NOT downgrade a row because CTD was empty or because target-organ data was sparse — grade each row
...
(direct linkage) OR whether the link is INFERRED from shared molecular targets (CTD gene overlap with
the AOP key events). A direct stressor listing is Strong/Moderate; a CTD-overlap-only link is Weak.
...
- **MEDIUM** — IARC Group 2B/3, OR GHS Cat 4, OR Weak AOP / CTD-overlap signals only.
...
report each with its issuing source. CTD inferred associations have a high false-positive rate →
grade them Weak unless corroborated by an AOP key event or an explicit stressor listing.
...
inferred from shared molecular targets (CTD overlap)?
...
(4) Toxicogenomic corroboration — does CTD gene-disease evidence corroborate the AOP's predicted
...
## 5. Toxicogenomics — CTD (if any)   (chemical–gene / chemical–disease | interaction/evidence type | AOP KE match | Grade (T1-T4) | Source)
