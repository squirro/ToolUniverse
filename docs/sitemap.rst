:orphan:

Documentation Sitemap
=====================

**Complete navigation of all ToolUniverse documentation**

Get Started
--------------

- :doc:`guide/python_guide` - Python Developer Guide
- :doc:`guide/building_ai_scientists/index` - Choose Your AI Agent Platform
- :doc:`guide/skills_showcase` - AI Agent Skills
- :doc:`guide/api_keys` - API Keys Configuration

Tutorials & Workflows
-------------------------

- :doc:`guide/index` - Tutorials Overview
- :doc:`guide/tooluniverse_case_study` - Case Study
- :doc:`guide/agentic_tools_tutorial` - Agentic Tools Tutorial
- :doc:`guide/literature_search_tools_tutorial` - Literature Search Tutorial
- :doc:`guide/literature_search_web_ui_tutorial` - Literature Search Web UI
- :doc:`guide/clinical_guidelines_tools` - Clinical Guidelines Tools
- :doc:`guide/euhealth_tools_tutorial` - EUHealth Tools Tutorial
- :doc:`guide/scientific_workflows` - Scientific Workflows
- :doc:`guide/expert_feedback` - Expert Feedback
- :doc:`guide/make_your_data_agent_searchable` - Make Your Data Agent Searchable
- :doc:`guide/visualization_tutorial` - Visualization Tutorial

️ Advanced Features
---------------------

- :doc:`guide/building_ai_scientists/compact_mode` - Compact Mode
- :doc:`guide/toolspace` - Toolspace
- :doc:`guide/hooks/index` - Hooks System
- :doc:`guide/cache_system` - Cache System
- :doc:`guide/examples` - Examples
- :doc:`guide/logging` - Logging
- :doc:`guide/openrouter_support` - OpenRouter Support
- :doc:`guide/streaming_tools` - Streaming Tools
- :doc:`guide/tools` - Tools
- :doc:`guide/vllm_support` - vLLM Support
- :doc:`guide/http_api` - HTTP API
- :doc:`help/wechat_community` - WeChat Community

Expand ToolUniverse
-----------------------

- :doc:`expand_tooluniverse/index` - Overview
- :doc:`expand_tooluniverse/quick_start` - Quick Start
- :doc:`expand_tooluniverse/local_tools/index` - Local Tools
- :doc:`expand_tooluniverse/remote_tools/index` - Remote Tools
- :doc:`expand_tooluniverse/contributing/index` - Contributing
- :doc:`expand_tooluniverse/architecture` - Architecture

Tools Catalog
-----------------

- :doc:`tools/tools_config_index` - Tools Configuration Index
- :doc:`tools/remote_tools` - Remote Tools

API Reference
-----------------

- :doc:`api/modules` - API Modules

Help & Reference
-------------------

- :doc:`reference/cli_tools` - CLI Tools
- :doc:`reference/environment_variables` - Environment Variables
- :doc:`reference/glossary` - Glossary
- :doc:`reference/data_sources` - Data Sources
- :doc:`help/index` - Help
- :doc:`expand_tooluniverse/reference/index` - Reference
- :doc:`about/index` - About

Python API Pages
----------------

- :doc:`guide/coding_api` - Coding API
- :doc:`guide/interaction_protocol` - Interaction Protocol
- :doc:`guide/loading_tools` - Loading Tools
- :doc:`guide/listing_tools` - Listing Tools
- :doc:`guide/finding_tools` - Finding Tools
- :doc:`guide/tool_caller` - Tool Caller
- :doc:`guide/tool_composition` - Tool Composition
